...
header('Content-disposition: attachment; filename=Cafe-Order-Report_'.$fds.' To '.$tds.'.xls');
}
?>
    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">D2D Cafe Order
              <div class="form-group floatright"> <a href="report_d2d_cafeorder.php?etype=1" target="_blank" class="btn btn-success  btn-mini">Print</a> <a href="report_d2d_cafeorder.php?etype=2" class="btn btn-success btn-mini">Excel</a>
                <?php /*?><a href="report_d2d_cafeorder.php?etype=3" target="_blank" class="btn btn-success  btn-mini">PDF</a>
...
?>
                  <script language="javascript">window.location.href="report_d2d_cafeorder.php";</script>
                  <?php } ?>
                </div>
              </div>
            </div>
        ...
        .addEventListener("click", () => {
            ...
            html2pdf().from(invoice).set(opt).save();
                    ...
                    <div id="invoice" class="page">
                      <center>
                        <h4>Cafe Order Booking Report</h4>
                      </center>
                  ...
                  <tbody>
                    <?php								 
						if($fd != '' && $td != '') { $date_qry = "and order_date between '$fd' and '$td'"; }
						if($venue_id != '') { $venue_qry = "and venue_id = '$venue_id'"; }
											 
						$ureee=("select * from order_master where red_oid = '0' $date_qry $venue_qry");
						$uree = mysqli_query($con,$ureee);
						while($uroww=mysqli_fetch_array($uree))
						{
						$red_oid = $uroww[0];
						$customer_mobile = $uroww['customer_mobile'];
						?>
                    <tr class="showtr">
                      ...
                      <td><?php echo $uroww['customer_name']; ?> </td>
                      <td><?php echo $uroww['customer_mobile']; ?> </td>
                      ...
                      <td> <a target="_blank" href="https://wa.me/91<?php echo $customer_mobile; ?>?text=Order Detail%0a%0aName : <?php echo $uroww['customer_name']; ?>%0a%0a<?php $data = mysqli_query($con,"select * from order_master where red_oid='$red_oid' order by ABS(id) asc"); while($urowx = mysqli_fetch_array($data)) { ?>
Date : <?php echo $urowx['barcode']; ?> | Rs. <?php echo $urowx['price']; ?> * <?php echo $urowx['qty']; ?> = <?php echo $urowx['amount']; ?>%0a<?php } ?>"><img src="images/whatsapp.png" width="30"></a></td>
                    </tr>
                    <?php } ?>
                  </tbody>
                </table>
