    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">Service Master</h2>
          </div>
        </div>
      </div>
    </div>
...
if ($_POST['insert']) 
{
  ...
  $sql=("INSERT INTO `service_master` ( `name`,  `seo_title`, `seo_keyword`, `seo_description`, `status`, `additional_info`) VALUES ( '".mysqli_real_escape_string($con,$_REQUEST['name'])."','".mysqli_real_escape_string($con,$_REQUEST['seo_title'])."', '".mysqli_real_escape_string($con,$_REQUEST['seo_keyword'])."', '".mysqli_real_escape_string($con,$_REQUEST['seo_description'])."','1', '".mysqli_real_escape_string($con,$_REQUEST['additional_info'])."')");	 
...
$z = mysqli_query($con,$sql);
	...
	if($img1 != '')
 {
    $uupQry="UPDATE service_master SET image='$img1'  WHERE id='$id'";
	$uuresult=mysqli_query($con,$uupQry);
 }

?>
    <script language="javascript">window.location.href="service_master.php";</script>
    <?php 
} 
 ...
 // exit();
    ...
    <?php  } ?>
    <?php 
					$ure=mysqli_query($con,"select * from service_master where id = '$id'");
            ...
            <div class="element-box">
              <div class="table-responsive">
                <table class="table " id="dataTables-example">
                  ...
                  <tbody>
                    <?php								 
						$ure=mysqli_query($con,"select * from service_master");
                    ...
                    <tr class="showtr">
                      ...
                      <td class="center"><a class="btn btn-mini btn-success" href="service_master.php?uid=<?php echo $urow[0]; ?>" title=""><i class="icon-pencil"></i></a> <a href="#" id="<?php echo $urow[0]; ?>" class="delete btn btn-mini btn-danger" title="Delete" ><i class="icon-trash"></i></a> </td>
                    </tr>
                    <?php } ?>
                  </tbody>
                </table>
...
{
$pid=$_POST['pid'];
$ure=mysqli_query($con,"select * from service_master where id = '$pid'");
$urow=mysqli_fetch_array($ure);
$ostatus = $urow['status'];
if($ostatus == '1') { $status = '0'; } else { $status = '1'; }
$uupQry="UPDATE service_master SET status='$status' WHERE id='$pid'";
$uuresult=mysqli_query($con,$uupQry);
}
?>
...
{
$id=$_POST['id'];
$delete = "DELETE FROM service_master WHERE id = '$id'";
