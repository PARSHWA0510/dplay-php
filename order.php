...
$master_detail = mysqli_fetch_array($master); ?>
          <tr>
            <td colspan="2"><strong><a href="<?php echo $siteurl_link; ?>/<?php echo $master_detail['seo_url']; ?>"><?php echo $master_detail['name']; ?></a></strong></td>
          </tr>
          <?php 
...
$master_detailc = mysqli_fetch_array($masterc); ?>
          <tr>
            <td colspan="2">&nbsp;&nbsp;<a href="court-book.php?id=<?php echo $master_detailc[0]; ?>"><?php echo $master_detailc['name']; ?></a></td>
          </tr>
          <?php 
...
?>
          <tr>
            <td colspan="2">&nbsp;&nbsp;&nbsp;&nbsp;<?php echo date("d-m-Y", strtotime($urowcd['court_date'])); ?></td>
          </tr>
          <?php 
...
while($urowcdc=mysqli_fetch_array($urecdc)) { ?>
      ...
      <div class="section">
		...
		$urowx=mysqli_fetch_array($urexy);
		?>
          <?php $charge1 = $urowx['charge1']; ?>
          <?php $charge2 = $urowx['charge2']; ?>
          <?php $charge3 = $urowx['charge3']; ?>
          ...
          <?php if($charge1 != '0') {?>
          <?php		
		  $ure=mysqli_query($con,"select * from charges_master where status = '1' limit 0,1");
		  $urow=mysqli_fetch_array($ure); ?>
          <div class="row"><span class="muted"><?php echo $urow['title1']; ?></span><strong>Rs. <?php echo $charge1; ?></strong></div>
          <?php } ?>
          <?php if($charge2 != '0') {?>
          <?php		
		  $ure=mysqli_query($con,"select * from charges_master where status = '1' limit 1,1");
		  $urow=mysqli_fetch_array($ure); ?>
          <div class="row"><span class="muted"><?php echo $urow['title1']; ?></span><strong>Rs. <?php echo $charge2; ?></strong></div>
          <?php } ?>
          <?php if($charge3 != '0') {?>
          <?php		
		  $ure=mysqli_query($con,"select * from charges_master where status = '1' limit 2,1");
		  $urow=mysqli_fetch_array($ure); ?>
          <div class="row"><span class="muted"><?php echo $urow['title1']; ?></span><strong>Rs. <?php echo $charge3; ?></strong></div>
          <?php } ?>
...
$master_detail = mysqli_fetch_array($master); ?>
      <div class="addresses" style="display:block">
        <div class="address"> <strong><?php echo $master_detail['name']; ?></strong>
          <div><?php echo $master_detail['address']; ?></div>
        </div>
      </div>
      <?php } ?>
    </section>
