    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">Manage Venue Photo | <a href="venue_manger.php" class="btn btn-primary" style="padding:2px 5px; font-size:14px">Back</a></h2>
          </div>
        </div>
      </div>
    </div>
...
if ($_POST['insert']) 
{
			...
			$fileCount1 = count($_FILES['img1']['name']);
			for($i = 0; $i < $fileCount1; $i++)
			{
        	 $img1x = $_FILES['img1']['name'][$i];
			 if($img1x != '') { 
        	 $img1 = rand(1,1000000).str_replace(" ","_",trim($_FILES['img1']['name'][$i]));
 			// Upload file to $path
  			move_uploaded_file($_FILES['img1']['tmp_name'][$i], $path.$img1);

			$sql=mysqli_query($con,"INSERT INTO venue_photo(`venue_id`,`status`,`image`) VALUES ('".mysqli_real_escape_string($con,$_GET['id'])."','1','$img1')");
 			}
			}
	
 	?>
                  <script language="javascript">window.location.href="venue_photo.php?id=<?php echo $_GET['id']; ?>";</script>
                  <?php 
  }
?>
                  <?php 
...
if(isset($_REQUEST['update'])) 
  {
	...
	if($img1 != '')
 {
    $uupQry="UPDATE venue_photo SET image='$img1'  WHERE id='$id'";
	...
	?>
                  ...
                  <?php  } ?>
                  <?php 
		$ure=mysqli_query($con,"select * from venue_photo where id = '$id'");
		$urow=mysqli_fetch_array($ure); ?>
                  <form autocomplete="off" name="form" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                      ...
                      <input class="form-control" readonly  required value="<?php echo $urowx['name']; ?>" name="name" type="text">
                    </div>
                    <div class="form-group">
                      <?php if($id) { ?>
                      ...
                      <?php } else { ?>
                      <label>Upload Multiple Photo <span class="required">*</span></label>
                      <input class="form-control" name="img1[]" multiple  type="file">
                      <?php } ?>
                      <?php $image = $urow['image']; if($image != '') { ?>
                      <img src="../images/<?php echo $image; ?>" width="150" style="margin-top:5px;">
                      <?php } ?>
                    </div>
                  ...
                  <div class="table-responsive">
                    <table class="table " id="dataTables-example">
                      ...
                      <tbody>
						...
						?>
                        <tr class="showtr">
                          <td><img src="../images/<?php echo $urow['image']; ?>" width="100"></td>
                          ...
                          <td class="center"><a class="btn btn-mini btn-success" href="venue_photo.php?id=<?php echo $_GET['id']; ?>&uid=<?php echo $urow[0]; ?>" title=""><i class="icon-pencil"></i></a> <a href="#" id="<?php echo $urow[0]; ?>" class="delete btn btn-mini btn-danger" title="Delete" ><i class="icon-trash"></i></a> </td>
                        </tr>
                        <?php } ?>
                      </tbody>
                    </table>
...
{
$pid=$_POST['pid'];
$ure=mysqli_query($con,"select * from venue_photo where id = '$pid'");
$urow=mysqli_fetch_array($ure);
$ostatus = $urow['status'];
if($ostatus == '1') { $status = '0'; } else { $status = '1'; }
$uupQry="UPDATE venue_photo SET status='$status' WHERE id='$pid'";
$uuresult=mysqli_query($con,$uupQry);
}
?>
...
{
$id=$_POST['id'];
$delete = "DELETE FROM venue_photo WHERE id = '$id'";
