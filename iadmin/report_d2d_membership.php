...
if ( $etype == '2' ) {
  ...
  header( 'Content-disposition: attachment; filename=Membership-Report_' . $fds . ' To ' . $tds . '.xls' );
}
?>
<?php
if ( $etype == '1' || $etype == '3' ) {
    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">D2D Membership Booking
              <div class="form-group floatright"> <a href="report_d2d_membership.php?etype=1" target="_blank" class="btn btn-success  btn-mini">Print</a> <a href="report_d2d_membership.php?etype=2" class="btn btn-success btn-mini">Excel</a>
                <?php /*?> < a href = "report_d2d_membership.php?etype=3"
                target = "_blank"
                class = "btn btn-success  btn-mini" > PDF < /a>
              ...
              <div class="element-wrapper">
                <div class="element-box">
                  ...
                  if ( isset( $_REQUEST[ 'search' ] ) ) {
                    ...
                    ?>
                  <script language="javascript">window.location.href="report_d2d_membership.php";</script>
                  <?php } ?>
                </div>
              </div>
            </div>
        ...
        .addEventListener("click", () => {
            ...
            html2pdf().from(invoice).set(opt).save();
                    ...
                    <div id="invoice" class="page">
                      <center>
                        <h4>Membership Booking Report</h4>
                      </center>
                      <table class="table " style="width:100%" cellpadding="0" cellspacing="0" border="1" id="dataTables-example">
                        ...
                        <tbody>
                          <?php
                          if ( $fd != '' && $td != '' ) {
                            $date_qry = "and DATE(order_datetime) between '$fd' and '$td'";
                          }
                          if ( $venue_id != '' ) {
                            $venue_qry = "and venue_id = '$venue_id'";
                          }

                          $ureee = ( "select * from user_membership where payment_status = '1' $date_qry $venue_qry" );
                          $uree = mysqli_query( $con, $ureee );
                          while ( $uroww = mysqli_fetch_array( $uree ) ) {
                            $package_id = $uroww[ 'package_id' ];
                            $urezc = mysqli_query( $con, "select * from package_user where id = '$package_id'" );
                          ...
                          <tr class="showtr">
                            ...
                            <td><?php
                            $package_id = $uroww[ 'package_id' ];
                            $urez = mysqli_query( $con, "select * from package_user where id = '$package_id'" );
                            $urowz = mysqli_fetch_array( $urez );
                            echo $urowz[ 'name' ];
                            ?></td>
                            <td><?php echo $urowz['price'];  $pricex[] = $urowz['price'];   ?></td>
                          </tr>
                          <?php } ?>
                        </tbody>
                        <tfoot>
                          <tr>
                            ...
                            <th> </th>
                            <th> </th>
                            <th> <?php echo array_sum($pricex); ?></th>
                          </tr>
                        </tfoot>
                      </table>
                    </div>
