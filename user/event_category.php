    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">Manage Event Category </h2>
          </div>
        </div>
      </div>
    </div>
...
if ($_POST['insert']) 
{
	...
	$name = $_REQUEST['name'];
	$ure=mysqli_query($con,"select * from event_category where name = '$name'");
	$urowno=mysqli_num_rows($ure);
	if($urowno > 0) 
	{ ?>
    <script>alert('Name Already Exist. Please Change');</script>
	<script language="javascript">window.location.href="event_category.php";</script>
    <?php } else { 
    $sql=mysqli_query($con,"INSERT INTO `event_category` (`user_id`,`name`, `status`, `image`, `orderlist`) VALUES ('$admin_id','".mysqli_real_escape_string($con,$_REQUEST['name'])."','1','$img1','".mysqli_real_escape_string($con,$_REQUEST['orderlist'])."')");	 
 	?>
                  <script language="javascript">window.location.href="event_category.php";</script>
                  <?php 
}  }
...
if(isset($_REQUEST['update'])) 
  {
	...
	$uupQry="UPDATE event_category SET name='".mysqli_real_escape_string($con,$_REQUEST['name'])."',orderlist='".mysqli_real_escape_string($con,$_REQUEST['orderlist'])."' WHERE id='$id'";
	...
	if($img1 != '')
 {
    $uupQry="UPDATE event_category SET image='$img1'  WHERE id='$id'";
	$uuresult=mysqli_query($con,$uupQry);
 }
	...
	?>
                  ...
                  <?php  } ?>
                  <?php 
		$ure=mysqli_query($con,"select * from event_category where id = '$id'");
                ...
                <div class="element-box">
                  <div class="table-responsive">
                    <table class="table " id="dataTables-example">
                      ...
                      <tbody>
                        <?php								 
						$ure=mysqli_query($con,"select * from event_category where user_id = '$admin_id'");
                        ...
                        <tr class="showtr">
                          ...
                          <td class="center"><a class="btn btn-mini btn-success" href="event_category.php?uid=<?php echo $urow[0]; ?>" title=""><i class="icon-pencil"></i></a> <a href="#" id="<?php echo $urow[0]; ?>" class="delete btn btn-mini btn-danger" title="Delete" ><i class="icon-trash"></i></a> </td>
                        </tr>
                        <?php } ?>
                      </tbody>
                    </table>
...
{
$pid=$_POST['pid'];
$ure=mysqli_query($con,"select * from event_category where id = '$pid'");
$urow=mysqli_fetch_array($ure);
$ostatus = $urow['status'];
if($ostatus == '1') { $status = '0'; } else { $status = '1'; }
$uupQry="UPDATE event_category SET status='$status' WHERE id='$pid'";
$uuresult=mysqli_query($con,$uupQry);
}
?>
