...
if ($_POST['insert']) 
{
  ...
  $sql=("INSERT INTO `features_master` ( `name`,  `seo_title`, `seo_keyword`, `seo_description`, `status`, `additional_info1`, `additional_info2`) VALUES ( '".mysqli_real_escape_string($con,$_REQUEST['name'])."','".mysqli_real_escape_string($con,$_REQUEST['seo_title'])."', '".mysqli_real_escape_string($con,$_REQUEST['seo_keyword'])."', '".mysqli_real_escape_string($con,$_REQUEST['seo_description'])."','1', '".mysqli_real_escape_string($con,$_REQUEST['additional_info1'])."', '".mysqli_real_escape_string($con,$_REQUEST['additional_info2'])."')");	 
...
$z = mysqli_query($con,$sql);
	...
	if($img1 != '')
 {
    $uupQry="UPDATE features_master SET image='$img1'  WHERE id='$id'";
	$uuresult=mysqli_query($con,$uupQry);
 }

?>
    <script language="javascript">window.location.href="features_master.php";</script>
    <?php 
} 
	...
	$uupQry="UPDATE features_master SET  `name` = '".mysqli_real_escape_string($con,$_REQUEST['name'])."', `seo_title` = '".mysqli_real_escape_string($con,$_REQUEST['seo_title'])."',`seo_keyword` = '".mysqli_real_escape_string($con,$_REQUEST['seo_keyword'])."',`seo_description` = '".mysqli_real_escape_string($con,$_REQUEST['seo_description'])."',`additional_info1` = '".mysqli_real_escape_string($con,$_REQUEST['additional_info1'])."',`additional_info2` = '".mysqli_real_escape_string($con,$_REQUEST['additional_info2'])."' WHERE id='$id'";
	$uuresult=mysqli_query($con,$uupQry);
 ...
 // exit();
    ...
    <?php  } ?>
    <?php 
					$ure=mysqli_query($con,"select * from features_master where id = '$id'");
            ...
            <div class="element-box">
              <form autocomplete="off"  name="form" method="post" enctype="multipart/form-data">
                <div class="row">
                  ...
                  <div class="col-sm-12">
                    <div class="form-group">
                      <label for=""> Long Information<span class="required">*</span></label>
                      <textarea name="additional_info2" class="input-xlarge" style="width:90%; height:150px"/>
                      <?php echo $urow['additional_info2']; ?>
                      </textarea>
					...
					//<![CDATA[
						...
						CKEDITOR.replace( 'additional_info2' );
					//]]>
					</script>
                    </div>
                  </div>
            ...
            <div class="element-box">
              <div class="table-responsive">
                <table class="table " id="dataTables-example">
                  ...
                  <tbody>
                    <?php								 
						$ure=mysqli_query($con,"select * from features_master");
                    ...
                    <tr class="showtr">
                      ...
                      <td class="center"><a class="btn btn-mini btn-success" href="features_master.php?uid=<?php echo $urow[0]; ?>" title=""><i class="icon-pencil"></i></a> <a href="#" id="<?php echo $urow[0]; ?>" class="delete btn btn-mini btn-danger" title="Delete" ><i class="icon-trash"></i></a> </td>
                    </tr>
                    <?php } ?>
                  </tbody>
                </table>
...
{
$pid=$_POST['pid'];
$ure=mysqli_query($con,"select * from features_master where id = '$pid'");
$urow=mysqli_fetch_array($ure);
$ostatus = $urow['status'];
if($ostatus == '1') { $status = '0'; } else { $status = '1'; }
$uupQry="UPDATE features_master SET status='$status' WHERE id='$pid'";
$uuresult=mysqli_query($con,$uupQry);
}
?>
...
{
$id=$_POST['id'];
$delete = "DELETE FROM features_master WHERE id = '$id'";
