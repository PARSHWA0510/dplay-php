...
header('Content-disposition: attachment; filename=Coach-Report_'.$fds.' To '.$tds.'.xls');
}
?>
    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">D2D Coach Booking
              <div class="form-group floatright"> <a href="report_d2d_coach.php?etype=1" target="_blank" class="btn btn-success  btn-mini">Print</a> <a href="report_d2d_coach.php?etype=2" class="btn btn-success btn-mini">Excel</a>
                <?php /*?><a href="report_d2d_coach.php?etype=3" target="_blank" class="btn btn-success  btn-mini">PDF</a>
                ...
                <div class="element-box">
                  <form autocomplete="off" onKeyDown="return event.key != 'Enter';" name="form" method="post" enctype="multipart/form-data">
                    <div class="row">
                      ...
                      <div class="col-sm-4">
                        <div class="form-group">
                          <label for="">Select Coach</label>
                          <br>
                          <select  class='form-control chosen_select' name="coach_id" style=" display:inline">
							...
							while($Fcoun=mysqli_fetch_array($coun)) { 
							 ?>
                            <option value="<?php echo $Fcoun[0]; ?>" <?php if($Fcoun[0] == $coach_id) echo 'selected="selected"';?>><?php echo $Fcoun['name']; ?></option>
                            <?php } ?>
                          </select>
                        </div>
                      </div>
...
?>
                  <script language="javascript">window.location.href="report_d2d_coach.php";</script>
                  <?php } ?>
                </div>
              </div>
            </div>
            ...
            html2pdf().from(invoice).set(opt).save();
                    ...
                    <div id="invoice" class="page">
                      ...
                      <table class="table " style="width:100%" cellpadding="0" cellspacing="0" border="1" id="dataTables-example">
                        ...
                        <tbody>
                          <?php	
						$uretbo=mysqli_query($con,"select * from user_order where coach_id = '0'");
						while($urowtbo=mysqli_fetch_array($uretbo))
						{
						$coach_batch_id = $urowtbo['coach_batch_id']; 
						$id = $urowtbo[0]; 
						
						$ureeex=("select * from coach_batch where id = '$coach_batch_id'"); 
						$ureex = mysqli_query($con,$ureeex);
						$urowwx=mysqli_fetch_array($ureex);
						$coach_id_store = $urowwx['coach_id'];	
						
						$uupQry="UPDATE user_order SET `coach_id` = '$coach_id_store' WHERE id = '$id'";
						$uuresult=mysqli_query($con,$uupQry);	
						}
						if($fd != '' && $td != '') { $date_qry = "and DATE(order_datetime) between '$fd' and '$td'"; }
						if($coach_id != '') { $coach_qry = "and coach_id = '$coach_id'"; }

						$uree=("select * from user_order where payment_status = '1' and order_type = 'coachbook' $date_qry $coach_qry");
						$ure = mysqli_query($con,$uree);
						while($uroww=mysqli_fetch_array($ure))
						{  ?>
                          <tr class="showtr">
						  ...
						  $urowz=mysqli_fetch_array($urez); echo $urowz['name'];  $coach_id = $urowz['coach_id']; $bid = $urowz[0]; ?>
                              <?php 
						  $urez=mysqli_query($con,"select * from user_master where id = '$coach_id'");
						  $urowz=mysqli_fetch_array($urez); $coach_name = $urowz['name']; ?>
                            </td>
                            <td><?php echo $coach_name; ?></td>
                            <td><?php echo date("d-m-Y", strtotime($urowz['start_date'])); ?> <br/>
                              <?php echo date("d-m-Y", strtotime($urowz['end_date'])); ?> </td>
                            <td><?php echo date("h:i A", strtotime($urowz['start_time'])); ?> <br/>
                              <?php echo date("h:i A", strtotime($urowz['end_time'])); ?> </td>
                            <td><?php 
						$urebd=mysqli_query($con,"select * from coach_batch_date where batch_id = '$bid' group by batch_day order by batch_date asc");
						while($urowbd=mysqli_fetch_array($urebd)) { ?>
						  ...
						  $urowz=mysqli_fetch_array($urez); echo $urowz['name']; ?>
                            ...
                            <td><?php echo $uroww['final_amount'];  $final_amountz[] = $uroww['final_amount']; ?></td>
                            <td><a href="../order-coach.php?order_id=<?php echo $uroww['order_id']; ?>" class="btn btn-success btn-mini" target="_blank"><i class="feather-printer me-2"></i>Print</a></td>
                          </tr>
                          <?php } ?>
                        </tbody>
                        <tfoot>
                          <tr>
                            ...
                            <th></th>
                            <th> </th>
                            <th><?php echo array_sum($final_amountz); ?> </th>
                            <th> </th>
                          </tr>
                        </tfoot>
                      </table>
