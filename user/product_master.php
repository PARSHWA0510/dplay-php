    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">Manage Cafe Product </h2>
          </div>
        </div>
      </div>
    </div>
...
if ($_POST['insert']) 
{
	...
	$ure=mysqli_query($con,"select * from product_master where item_name = '$item_name'");
	$urowno=mysqli_num_rows($ure);
	if($urowno > 0) 
	{ ?>
    <script>alert('Name Already Exist. Please Change');</script>
	<script language="javascript">window.location.href="product_master.php";</script>
    <?php } else { 
    $sql=mysqli_query($con,"INSERT INTO `product_master` (`item_name`,`sale_rate`, `status`) VALUES ('".mysqli_real_escape_string($con,$_REQUEST['item_name'])."','".mysqli_real_escape_string($con,$_REQUEST['sale_rate'])."','1')");	 
 	?>
                  <script language="javascript">window.location.href="product_master.php";</script>
                  <?php 
}  }
...
if(isset($_REQUEST['update'])) 
  {
	...
	$uupQry="UPDATE product_master SET item_name='".mysqli_real_escape_string($con,$_REQUEST['item_name'])."',sale_rate='".mysqli_real_escape_string($con,$_REQUEST['sale_rate'])."' WHERE id='$id'";
	...
	?>
                  ...
                  <?php  } ?>
                  <?php 
		$ure=mysqli_query($con,"select * from product_master where id = '$id'");
		$urow=mysqli_fetch_array($ure); ?>
                  <form autocomplete="off" name="form" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                      <label> Name <span class="required">*</span></label>
                      <input class="form-control" autofocus  required value="<?php echo $urow['item_name']; ?>" name="item_name" type="text">
                    </div>
                    <div class="form-group">
                      <label> Price <span class="required">*</span></label>
                      <input class="form-control"  required value="<?php echo $urow['sale_rate']; ?>" name="sale_rate" type="text">
                    </div>
                ...
                <div class="element-box">
                  <div class="table-responsive">
                    <table class="table " id="dataTables-example">
                      ...
                      <tbody>
                        <?php								 
						$ure=mysqli_query($con,"select * from product_master");
						while($urow=mysqli_fetch_array($ure))
						 {
						?>
                        <tr class="showtr">
                          <td><?php echo $urow['item_name']; ?> </td>
                          <td><?php echo $urow['sale_rate']; ?> </td>
                          ...
                          <td class="center"><a class="btn btn-mini btn-success" href="product_master.php?uid=<?php echo $urow[0]; ?>" title=""><i class="icon-pencil"></i></a> <a href="#" id="<?php echo $urow[0]; ?>" class="delete btn btn-mini btn-danger" title="Delete" ><i class="icon-trash"></i></a> </td>
                        </tr>
                        <?php } ?>
                      </tbody>
                    </table>
...
$uupQry="UPDATE product_master SET status='$status' WHERE id='$pid'";
$uuresult=mysqli_query($con,$uupQry);
}
?>
