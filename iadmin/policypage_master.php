    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">Policy Page Master</h2>
          </div>
        </div>
      </div>
    </div>
...
$z = mysqli_query($con,$sql);
	...
	if($img1 != '')
 {
    $uupQry="UPDATE policypage_master SET image='$img1'  WHERE id='$id'";
	$uuresult=mysqli_query($con,$uupQry);
 }

?>
    <script language="javascript">window.location.href="policypage_master.php";</script>
    <?php 
} 
 ...
 // exit();
    ...
    <?php  } ?>
    <?php 
					$ure=mysqli_query($con,"select * from policypage_master where id = '$id'");
                ...
                <div class="row">
                  <div class="col-sm-4">
                    <div class="form-group">
                      <label for=""> Policy Page<span style="color:#FF0000">*</span></label>
                      <select  class='form-control chosen_select' required name="page_name">
                        <option value="">Please Select</option>
                        <option value="Refund Policy" <?php if('Refund Policy' == $urow['page_name']) echo 'selected="selected"';?>>Refund Policy</option>
                        <option value="Privacy Policy" <?php if('Privacy Policy' == $urow['page_name']) echo 'selected="selected"';?>>Privacy Policy</option>
                        <option value="Shipping Policy" <?php if('Shipping Policy' == $urow['page_name']) echo 'selected="selected"';?>>Shipping Policy</option>
                        <option value="Terms and Conditions" <?php if('Terms and Conditions' == $urow['page_name']) echo 'selected="selected"';?>>Terms and Conditions</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div class="col-sm-12">
                    <div class="form-group">
                      <label for=""> Information<span class="required">*</span></label>
            ...
            <div class="element-box">
              <div class="table-responsive">
                <table class="table " id="dataTables-example">
                  ...
                  <tbody>
                    <?php								 
						$ure=mysqli_query($con,"select * from policypage_master");
                    ...
                    <tr class="showtr">
                      ...
                      <td class="center"><a class="btn btn-mini btn-success" href="policypage_master.php?uid=<?php echo $urow[0]; ?>" title=""><i class="icon-pencil"></i></a> <a href="#" id="<?php echo $urow[0]; ?>" class="delete btn btn-mini btn-danger" title="Delete" ><i class="icon-trash"></i></a> </td>
                    </tr>
                    <?php } ?>
                  </tbody>
                </table>
...
{
$pid=$_POST['pid'];
$ure=mysqli_query($con,"select * from policypage_master where id = '$pid'");
$urow=mysqli_fetch_array($ure);
$ostatus = $urow['status'];
if($ostatus == '1') { $status = '0'; } else { $status = '1'; }
$uupQry="UPDATE policypage_master SET status='$status' WHERE id='$pid'";
$uuresult=mysqli_query($con,$uupQry);
}
?>
...
{
$id=$_POST['id'];
$delete = "DELETE FROM policypage_master WHERE id = '$id'";
