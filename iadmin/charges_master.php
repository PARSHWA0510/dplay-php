...
<div class="wrapper">
  ...
  <div class="container mt-4 main-container">
	...
	$uupQry="UPDATE charges_master SET  `title1` = '".mysqli_real_escape_string($con,$_REQUEST['title1'])."', `title2` = '".mysqli_real_escape_string($con,$_REQUEST['title2'])."' WHERE id='$id'";
	$uuresult=mysqli_query($con,$uupQry);

 // exit();
	?>
    <script language="javascript">window.location.href="charges_master.php";</script>
    <?php  } ?>
    <?php 
					$ure=mysqli_query($con,"select * from charges_master where id = '$id'");
                ...
                <div class="row">
                  <div class="col-sm-6">
                    <div class="form-group">
                      <label for=""> Charges Name<span style="color:#FF0000">*</span></label>
                      <input class="form-control" name="title1"  required value="<?php echo $urow['title1']; ?>" type="text">
                    </div>
                  </div>
                  <div class="col-sm-6">
                    <div class="form-group">
                      <label for=""> Charges (%)<span style="color:#FF0000">*</span></label>
            ...
            <div class="element-box">
              <div class="table-responsive">
                <table class="table " id="dataTables-example">
                  ...
                  <tbody>
                    <?php								 
						$ure=mysqli_query($con,"select * from charges_master");
                    ...
                    <tr class="showtr">
                      ...
                      <td class="center"><a class="btn btn-mini btn-success" href="charges_master.php?uid=<?php echo $urow[0]; ?>" title=""><i class="icon-pencil"></i></a> </td>
                    </tr>
                    <?php } ?>
                  </tbody>
                </table>
...
{
$pid=$_POST['pid'];
$ure=mysqli_query($con,"select * from charges_master where id = '$pid'");
$urow=mysqli_fetch_array($ure);
$ostatus = $urow['status'];
if($ostatus == '1') { $status = '0'; } else { $status = '1'; }
$uupQry="UPDATE charges_master SET status='$status' WHERE id='$pid'";
$uuresult=mysqli_query($con,$uupQry);
}
?>
              </div>
