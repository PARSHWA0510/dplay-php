...
header('Content-disposition: attachment; filename=Income-Expense-Report_'.$fds.' To '.$tds.'.xls');
}
?>
    ...
    <div class="row">
      <div class="container my-3 main-container">
        <div class="row align-items-center">
          <div class="col">
            <h2 class="content-color-primary page-title">D2D Income/Expense Report
              <div class="form-group floatright"> <a href="report_d2d_income_expense.php?etype=1" target="_blank" class="btn btn-success  btn-mini">Print</a> <a href="report_d2d_income_expense.php?etype=2" class="btn btn-success btn-mini">Excel</a>
                <?php /*?><a href="report_d2d_income_expense.php?etype=3" target="_blank" class="btn btn-success  btn-mini">PDF</a>
...
?>
                  <script language="javascript">window.location.href="report_d2d_income_expense.php";</script>
                  <?php } ?>
                </div>
              </div>
            </div>
                    ...
                    <div id="invoice" class="page">
                      ...
                      <table class="table" style="width:100%" cellpadding="0" cellspacing="0" border="1" id="dataTables-example">
						...
						if($venue_id != '') { $venue_qry = "and venue_id = '$venue_id'"; }
						 ?>
                          <tr class="showtr">
                            <td><?php $ureee=("select SUM(court_price) from user_court where payment_status = '1' and court_date between '$fd' and '$td' $venue_qry");
						$uree = mysqli_query($con,$ureee);
						$uroww=mysqli_fetch_array($uree); echo $i1 = $uroww['SUM(court_price)'];?>
                            </td>
                            <td><?php $ureee=("select SUM(package_price) from user_membership where payment_status = '1' and DATE(order_datetime) between '$fd' and '$td' $venue_qry");
						$uree = mysqli_query($con,$ureee);
						$uroww=mysqli_fetch_array($uree); echo $i2 = $uroww['SUM(package_price)'];?>
                            </td>
                            <td><?php $ureee=("select SUM(price) from equipment_rented where DATE(ins_datetime) between '$fd' and '$td' $venue_qry");
						$uree = mysqli_query($con,$ureee);
						$uroww=mysqli_fetch_array($uree); echo $i3 = $uroww['SUM(price)'];?>
                            </td>
                            <td><?php $ureee=("select SUM(final_amount) from order_master where red_oid = '0' and order_date between '$fd' and '$td' $venue_qry");
						$uree = mysqli_query($con,$ureee);
						$uroww=mysqli_fetch_array($uree); echo $i4 = $uroww['SUM(final_amount)'];?>
                            </td>
                            <td><?php $ureee=("select SUM(total) from event_booked where id != '0' and DATE(booking_datetime) between '$fd' and '$td' $venue_qry");
						$uree = mysqli_query($con,$ureee);
						$uroww=mysqli_fetch_array($uree); echo $i5 = $uroww['SUM(total)'];?>
                            </td>
                            <td><?php $ureee=("select SUM(total) from coach_booked where id != '0' and DATE(booking_datetime) between '$fd' and '$td' $venue_qry");
						$uree = mysqli_query($con,$ureee);
						$uroww=mysqli_fetch_array($uree); echo $i6 = $uroww['SUM(total)'];?>
                            </td>
                            <td><?php $ureee=("select SUM(amount) from expense_master where id != '0' and payment_date between '$fd' and '$td' $venue_qry");
						$uree = mysqli_query($con,$ureee);
						$uroww=mysqli_fetch_array($uree); echo $e1 = $uroww['SUM(amount)'];?>
                            </td>
